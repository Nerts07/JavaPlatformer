package com.platformer2d.world.customgamemap;
...
public class CustomGameMapLoader
{
    ...
    public static CustomGameMapData loadMap(String id, String name)
    {
        ...
        FileHandle file = Gdx.files.local("maps/" + id + ".maps");
        if (file.exists())
        {
    ...
    public static CustomGameMapData generateRandomMap (String id, String name)
    {
        ...
        for (int row = 0; row < SIZE; row++)
        {
            for (int col = 0; col < SIZE; col++)
            {
                ...
                {
                    if (random.nextInt(50) == 0)//   1/50 chance of generate cloud for each id at sky
